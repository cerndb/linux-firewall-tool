...
class ManageHosts(object):
    ...
    def parse_config_file(self, deploy=False, no_default_config=False, update_sets=False, allow=False,
                          drop_all=False, generate_files=False, check_matches=False, update_list=None, exclude_list=None):
        """
        ...
        for sect in self.sections:
            ...
            if hostname in machines:
                ...
                if check_matches:
                    ...
                    print 'Call iptables manager'
                    print 'update_list, exclude_list', update_list, exclude_list
                    iptables_manager(None, config_files_list, default_interface, no_default_config, allow, drop_all,
                                     update_sets, deploy, generate_files, update_list, exclude_list)
                    sys.exit(0)
            else:
...
###########################################################################################################
    ...
    def host_file_mapping(self):
        ...
        graph.append('graph [fontsize=14 fontname="Verdana" compound=true];')
        graph.append('node [shape=record fontsize=12 fontname="Verdana"];')
        graph.append('ranksep="1.0"')
        graph.append('nodesep="0.5"')
        ...
        counter = 0
        for sect in self.sections:
            subgraphs.append('subgraph cluster_' + str(counter) + '{')
            subgraphs.append('style=filled;')
            subgraphs.append('color=lightgrey;')
            subgraphs.append('node [style=filled, color=cyan, shape=oval];')
            subgraphs.append('label = "' + sect + '";')
            ...
            if counter % 2 == 0:
                subgraphs.append('"cluster_' + str(counter) + '_next"[label="next"];')

            connection = '"cl-' + sect + '" -> '

            for _file_ in config_folder_files:
                subgraphs.append('"cluster_' + str(counter) + '_' + _file_ + '"[label="' + _file_ + '"];')
                connection += '"cluster_' + str(counter) + '_' + _file_ + '",'
            ...
            connection += ';'
            subgraphs.append(connection)
            subgraphs.append('"cl-' + sect + '" -> {"cluster_' + str(counter) + '_next"[label="next"]};')
            subgraphs.append('}')
            counter += 1

        graph.extend(subgraphs)
        ...
        try:
            file_path = "/var/tmp/firewall_files"
            with open(file_path + '/host_map.dot', 'w') as file_handler:
                file_handler.write("# Generated by host_manager.py\n")
                file_handler.write("#Use with https://mdaines.github.io/viz.js/\n")
                for line in graph:
                    file_handler.write(line+'\n')
            file_handler.close()
            print "File is located at " + file_path + '/host_map.dot'
        except:
...
#                 break
#         if check_landb_set == '':
#             print 'landb_set_check = "NETWORK_SET"  is missing from the config file'
#
#         check_landb_value = ''
#         for line in open(project_folder + '/default_conf_files' + '/configuration_info.cfg', 'r').readlines():
...
#                 break
#         if check_landb_value == '':
#             print 'landb_set_values = "HOST_IN_SET"  is missing from the config file'
#
#     except:
...
#         sys.exit(1)
#
#     if (check_landb_set != '') and (check_landb_value != ''):
#
#         check_value = netgroup_set_extractor('ip', check_landb_set, None, None, False, False)
#
#         if check_value[0][0] in check_landb_value:
#             valid_landb = True
#
#     project_folder = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
#
...
def main():
    ...
    parser.add_argument('--map_hostfile', action='store_true', help='Generates dot language code in order to visualize '
                                                                    'host file contents')
    ...
    if valid:
        if args.config:
            ...
            if args.map_hostfile:
                host_manager.host_file_mapping()
            else:
                host_manager.parse_config_file(deploy=args.deploy, no_default_config=args.no_default_config,
                                               update_sets=args.update_sets, allow=args.allow,
